...
use App\Exports\ReExaminationExport;
...
class ReexaminationController extends VoyagerBaseController
{
  ...
  public function index(Request $request)
  {
    ...
    $newData = ReExamination::with(['classroom'])->get();
    $newData->map(function($item, $key) {
       $newItem =  $item->getRelation('classroom')
              ->load('retestSchedule')->getRelation('retestSchedule');
      $newItem->map(function($subIem, $subKey) {
         return $subIem->load('room');
      });
      return $newItem;
    });
    $data = $newData->unique('classroom_id');
    ...
    public function export(Request $request, $slug) {
      return Excel::download(new ReExaminationExport($slug), 'danh-sach-sinh-vien.xlsx');
    }
}
